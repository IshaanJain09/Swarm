...
class FormationAdapter:
    def __init__(self, m, d, formation_system=None):
        ...
        # Initialize SpringDamperSystem with correct parameters
        if formation_system is None:
            self.formation = SpringDamperSystem(K=5.0, B=2.0, M=6.0, dt=m.opt.timestep)
        else:
            self.formation = formation_system
        
        # Register followers and store their offsets separately
        ...
        for rid in [2, 3, 4, 5]:
            self.formation.register_follower(rid)
    ...
    def get_follower_target(self, follower_id):
        ...
        if follower_id not in self.formation_offsets or len(self.leader_path) < 2:
            # Not enough path history, just use offset from current position
            offset = self.formation_offsets.get(follower_id, np.array([0.0, 0.0]))
    ...
    def compute_formation_motor_speeds(self, follower_id, current_time):
        target = self.get_follower_target(follower_id)
        
        try:
            ctrl = self.controllers[follower_id]
            follower_pos, _, follower_yaw = self.get_robot_state(follower_id)
            
            # Calculate simple navigation to target
            to_target = target - follower_pos
            ...
            heading_error = angle_to_target - follower_yaw
            ...
            turn_command = 4.0 * heading_error
            forward_speed = 8.0 if abs(heading_error) < 0.3 else 4.0
            ...
            self.logger.error(f"Error computing speeds for robot {follower_id}: {e}")
            return 0.0, 0.0

    def debug_print(self):
        self.logger.info("FormationAdapter initialized")
        self.logger.info(f"  Followers: {list(self.formation.robot_states.keys())}")
        for rid in self.formation.robot_states.keys():
            offset = self.formation_offsets.get(rid, np.array([0.0, 0.0]))
...
# ===== PLOTTING FUNCTIONS =====
def plot_trajectory_map(robot_positions, data_indices, victim_positions, found_victims, victim_info):
    """Plot trajectory map in a separate window"""
    colors = ['red', 'blue', 'green', 'orange', 'purple']
    
    fig_map = plt.figure(figsize=(14, 10))
    ax_map = fig_map.add_subplot(111)
    
    # Normalize victim_positions input
    victim_dict = {}
    ...
    if all_x and all_y:
        ...
        xspan = max(1e-3, maxx - minx)
        yspan = max(1e-3, maxy - miny)
        pad = max(0.5, 0.12 * max(xspan, yspan))
    ...
    ax_map.set_title('Swarm Search & Rescue - Robot Trajectories', fontsize=16, fontweight='bold')
    ...
    for rid in [1, 2, 3, 4, 5]:
        ...
        if idx > 0:
            ...
            if positions.shape[0] > 0:
                ax_map.plot(positions[:, 0], positions[:, 1], color=colors[rid-1],
                            linewidth=2, alpha=0.6, label=f'Robot {rid}')
                # Draw circle for the last position
                last_pos = positions[-1]
                ax_map.plot(last_pos[0], last_pos[1], 'o', color=colors[rid-1], 
                            markersize=12, markeredgecolor='black', markeredgewidth=2)
                
                # Draw circle for the starting position (lighter)
                start_pos = positions[0]
                ax_map.plot(start_pos[0], start_pos[1], 'o', color=colors[rid-1], 
    ...
    for vic_name, vic_pos in victim_dict.items():
        ...
        # Extract victim number from name
        try:
            vic_num = int(vic_name.replace('victim', ''))
        except:
            continue

        marker_x = float(vic_pos[0])
        marker_y = float(vic_pos[1])
        if vic_name in found_victims:
            ax_map.plot(marker_x, marker_y, '*', color='blue', markersize=35,
                        markeredgecolor='darkblue', markeredgewidth=3, alpha=0.9)
            ax_map.text(marker_x + 0.1, marker_y + 0.1, f'V{vic_num}\n✓FOUND',
                        fontsize=10, fontweight='bold', color='darkblue')
        else:
            ax_map.plot(marker_x, marker_y, '*', color='red', markersize=35,
                        markeredgecolor='darkred', markeredgewidth=3, alpha=0.9)
            ax_map.text(marker_x + 0.1, marker_y + 0.1, f'V{vic_num}\n✗MISSING',
                        fontsize=10, fontweight='bold', color='darkred')

    stats_text = f"Found: {len(found_victims)}/{len(victim_dict)}\n"
    stats_text += f"Success: {len(found_victims)/len(victim_dict)*100:.0f}%" if len(victim_dict) > 0 else "Success: 0%"
    ax_map.text(0.02, 0.98, stats_text, transform=ax_map.transAxes,
                fontsize=14, verticalalignment='top', fontweight='bold',
                bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.9))

    ax_map.legend(loc='upper left', bbox_to_anchor=(1.02, 1), fontsize=11, frameon=True)
